...
while [[ $# -gt 0 ]]; do
    case $1 in
        --help|-h)
            ...
            echo "  - NixOS configuration (configuration.nix)"
            echo "  - Hyprland configuration"
            echo "  - Waybar configuration"
            echo "  - Kitty terminal configuration"
            echo
            echo "Note: hardware-configuration.nix is not touched by this script."
            echo "      Generate it manually with: sudo nixos-generate-config"
            exit 0
            ;;
        *)
...
check_nix_syntax() {
    ...
    # Check for duplicate VirtualBox settings in configuration.nix
...
install_system_configs() {
    ...
    log_success "System configuration installed successfully"
    log_info "Note: hardware-configuration.nix was not modified"
    log_info "If you need to generate it: sudo nixos-generate-config"
...
final_report() {
    ...
    if [[ -f "/etc/nixos/hardware-configuration.nix" ]]; then
        ...
        echo "    Run: sudo nixos-generate-config to create it"
    fi
